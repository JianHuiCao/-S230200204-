...
def apply_loads(num_nodes, load_node_index, q, thickness):
    # 施加载荷
    F = np.zeros(2 * num_nodes)
    F[2 * load_node_index + 1] = -q * length * thickness / 2  # 上端面施加均布载荷
    return F
...
K = apply_boundary_conditions(K, nodes, [0])  # 左端面固定
F = apply_loads(num_nodes, num_nodes - 1, q, thickness)  # 上端面施加均布载荷
